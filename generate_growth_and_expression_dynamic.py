...
def same_adder_shape(f,X,W):
    ...
    Y = np.array(Y)
    ind = np.array([j.shape[0]-h.shape[0] for j,h in zip(Y,X)])==0
    return Y[ind],X[ind],W[ind]
def ensure_dimesnions(X,Y,Z,W):
    """Ensure list of arrays all have same dimensions"""
    X = np.array(X)
    Z = np.array(Z)
    W = np.array(W)
    Y = np.array(Y)
    ind = np.array([y.shape[0]-x.shape[0]-w.shape[0]-z.shape[0] for y,x,w,z in\
                    zip(Y,X,W,Z)])==0
    return X[ind],Y[ind],Z[ind],W[ind]
...
similar_frame(ml,gl,sl2,sm2,mq,gq,sq2,DV,beta,shape=(1,1000),dtsim=1,gfp_sym=True):
    ...
    for i in range(W.shape[0]):
        ...
        if gfp_sym:
            f,X,Ww = same_adder_shape(F[i,:],X,Ww)
            G = gfp_dynamics(f,X,C0,beta,dtsim)
            #print(len(f),len(X),len(G),len(Ww))
        else:
            X = X[:-2]; Ww=Ww[:-2]

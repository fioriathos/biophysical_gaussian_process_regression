...
def d_mean_x(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    ...
    if key=='sx2': dsx2_ = 0
    if key == 'sg2': dsg2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
    if key=='b': db_ = 0
...
def d_mean_l(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key == 'ml': dml_ = 1 - np.exp(-gl*t)
    if key == 'gl': dgl_ = -t*(bl-ml)*np.exp(-gl*t)
    if key == 'sl2': dsl2_ = 0
    if key == 'mq': dmq_ = 0
    if key == 'gq': dgq_ = 0
    if key == 'sq2': dsq2_ = 0
    ...
    if key == 'b': db_= 0
    if key == 'sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdxg_ = 0
    return {'ml':dml_,'gl':dgl_,'sl2':dsl2_,'mq':dmq_,'gq':dgq_,'sq2':dsq2_\
		,'sx2':dsx2_,'sg2':dsg2_,'b':db_,'sdx2':dsdx2_,'sdg2':dsdg2_}
def mean_q(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b):
    return mq+(bq-mq)*np.exp(-gq*t)
def d_mean_q(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key == 'ml': dml_ =0
    if key == 'gl': dgl_ =0
    if key == 'sl2': dsl2_ = 0
    if key == 'mq': dmq_ = 1 - np.exp(-gq*t)
    if key == 'gq': dgq_ = -t*(bq-mq)*np.exp(-gq*t)
    if key == 'sq2': dsq2_ = 0
    if key == 'sx2': dsx2_ = 0
    if key == 'sg2': dsg2_ = 0
    if key == 'b': db_ = 0
...
def d_cov_xg(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_x,dnm_g,key):
    ...
    if key=='b':
        db_ =  (-(((bg*bl + Cgl)*t)/Exp(b*t)) + ((bg*bl + Cgl)*t)/Exp((b + gl)*t) - ((bg*bx + Cxg)*gl*t)/Exp(b*t) + (bg*ml*t)/Exp(b*t) - \
         ...
         Clq*(-1 - Cxl*tau + ml*tau - Cll*tau**2 + Exp(gl*t)*\
         (1 + bx*gl*tau + Cxx*gl*tau - ml*tau + gl*ml*t*tau + Cll*tau**2 + Cxl*tau*(1 + gl*tau)))),0,t)[0])/gl- nm[1,0]*dnm_x['b']- nm[0,0]*dnm_g['b']
    return {'ml':dml_,'gl':dgl_,'sl2':dsl2_,'mq':dmq_,'gq':dgq_,'sq2':dsq2_\
		,'sx2':dsx2_,'sg2':dsg2_,'b':db_,'sdx2':dsdx2_,'sdg2':dsdg2_}
...
def d_cov_xl(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = (-(Cxl*Exp(gl*t)*gl**3*t) - (-1 + Exp(gl*t))*sl2*(-1 + Exp(gl*t) - gl*t) -\
			 Cll*gl*(-1 - 2*gl*t + Exp(gl*t)*(1 + gl*t)))/(Exp(2*gl*t)*gl**3)
    if key=='sl2': dsl2_ = 1/(2*gl**2)*(1-Exp(-gl*t))**2
    if key=='mq': dmq_ = 0
    if key=='gq': dgq_ = 0
    if key=='sq2': dsq2_ = 0
    if key=='sx2': dsx2_ = 0
...
def d_cov_xq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = (Clq*(1 - Exp(gl*t) + gl*t))/(Exp((gl + gq)*t)*gl**2)
    if key=='sl2': dsl2_ = 0
    if key=='mq': dmq_ = 0
    if key=='gq': dgq_ =-(((Clq*(-1 + Exp(gl*t)) + Cxq*Exp(gl*t)*gl)*t)/(Exp((gl + gq)*t)*gl))
    if key=='sq2': dsq2_ = 0
    if key=='sx2': dsx2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
...
def d_cov_gl(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_l,dnm_g,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ =bg/Exp(b*t) - bg/Exp((b + gl)*t) +\
		   quad(lambda tau:  Exp(bx + Cxx/2. - b*t - gl*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)*(-1 + Exp(gl*t))*\
			(bq + Cxq - mq + Exp(gq*tau)*mq + Clq*tau),0,t)[0]- nm[1,0]*dnm_l[key]- nm[2,0]*dnm_g[key]
    if key=='gl': dgl_ =         t*(-((bg*bl + Cgl)/Exp((b + gl)*t)) + (bg*ml)/Exp((b + gl)*t) +\
	    quad(lambda tau: Exp(bx + Cxx/2. - b*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)*ml*(bq + Cxq - mq + Exp(gq*tau)*mq + Clq*tau) - \
	    Exp(bx + Cxx/2. - b*t - gl*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)*\
	    ((bq + Cxq + (-1 + Exp(gq*tau))*mq)*(Cxl + (-1 + Exp(gl*t))*ml + Cll*tau) + bl*(bq + Cxq - mq + Exp(gq*tau)*mq + Clq*tau) + \
            Clq*(1 + Cxl*tau + (-1 + Exp(gl*t))*ml*tau + Cll*tau**2)),0,t)[0])- nm[1,0]*dnm_l[key]- nm[2,0]*dnm_g[key]
    if key=='sl2': dsl2_ = 0
    if key=='mq': dmq_ = quad( lambda tau: Exp(bx + Cxx/2. - b*t - gl*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)\
			*(-1 + Exp(gq*tau))*(bl + Cxl - ml + Exp(gl*t)*ml + Cll*tau),0,t)[0]- nm[1,0]*dnm_l[key]- nm[2,0]*dnm_g[key]
    if key=='gq': dgq_ =  quad(lambda tau: -(Exp(bx + Cxx/2. - b*t - gl*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)*tau*\
			((bq + Cxq - mq)*(Cxl + (-1 + Exp(gl*t))*ml + Cll*tau) + bl*(bq + Cxq - mq + Clq*tau) +\
		   	Clq*(1 + Cxl*tau + (-1 + Exp(gl*t))*ml*tau + Cll*tau**2))),0,t)[0]- nm[1,0]*dnm_l[key]- nm[2,0]*dnm_g[key]
    if key=='sq2': dsq2_ = 0
    if key=='sx2': dsx2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
    if key == 'sg2': dsg2_ = 0
    if key == 'b': db_ = -(((bg*bl + Cgl)*t)/Exp((b + gl)*t)) - (bg*ml*t)/Exp(b*t) + (bg*ml*t)/Exp((b + gl)*t) +\
		      	quad(lambda tau:Exp(bx + Cxx/2. - b*t - gl*t + (b + bl + Cxl - gq)*tau + (Cll*tau**2)/2.)*(-t + tau)*\
		    ((bq + Cxq + (-1 + Exp(gq*tau))*mq)*(Cxl + (-1 + Exp(gl*t))*ml + Cll*tau) + bl*(bq + Cxq - mq + Exp(gq*tau)*mq + Clq*tau) +\
		    Clq*(1 + Cxl*tau + (-1 + Exp(gl*t))*ml*tau + Cll*tau**2)),0,t)[0]- nm[1,0]*dnm_l[key]- nm[2,0]*dnm_g[key]
    return {'ml':dml_,'gl':dgl_,'sl2':dsl2_,'mq':dmq_,'gq':dgq_,'sq2':dsq2_\
...
def d_cov_gq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_q,dnm_g,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = 0
    if key=='sl2': dsl2_ = 0
    if key=='mq': dmq_ =         bg/Exp(b*t) - bg/Exp((b + gq)*t) +\
		   quad(lambda tau: Exp(bx + Cxx/2. - b*t - gq*t + b*tau + bl*tau + Cxl*tau - gq*tau + (Cll*tau**2)/2.)*\
		(bq*(-2 + Exp(gq*t) + Exp(gq*tau)) + (-1 + Exp(gq*tau))*(Cxq + (-1 + Exp(gq*t))*mq + Clq*tau) +\
		(-1 + Exp(gq*t))*(Cxq + (-1 + Exp(gq*tau))*mq + Clq*tau)),0,t)[0]- nm[1,0]*dnm_q[key]- nm[3,0]*dnm_g[key]
    if key=='gq': dgq_ = -((bg*bq*t)/Exp((b + gq)*t)) - (Cgq*t)/Exp((b + gq)*t) + (bg*mq*t)/Exp((b + gq)*t) + \
		quad(lambda tau: (Exp(bx + Cxx/2. - b*t + b*tau + bl*tau - gq*(t + tau) + (tau*(2*Cxl + Cll*tau))/2.)*sq2*\
		(1 + gq*(t + tau) + Exp(2*gq*tau)*(-1 + gq*(-t + tau))))/(2.*gq**2) + \
		Exp(bx + Cxx/2. - b*t - gq*t + b*tau + bl*tau + Cxl*tau - gq*tau + (Cll*tau**2)/2.)*\
		(-(bq**2*(t + tau)) - Cqq*(t + tau) - (Cxq - mq + Clq*tau)*\
		((-1 + Exp(gq*tau))*mq*t + (-1 + Exp(gq*t))*mq*tau + Cxq*(t + tau) + Clq*tau*(t + tau)) - \
		bq*((-2 + Exp(gq*tau))*mq*t + (-2 + Exp(gq*t))*mq*tau + 2*Cxq*(t + tau) + 2*Clq*tau*(t + tau))),0,t)[0]- nm[1,0]*dnm_q[key]- nm[3,0]*dnm_g[key]
    if key=='sq2': dsq2_ =quad(lambda tau: (Exp(bx - b*t + b*tau + bl*tau + (Cxx + tau*(2*Cxl + Cll*tau))/2.)\
			    *(Exp(gq*(-t + tau)) - Exp(-(gq*(t + tau)))))/(2.*gq),0,t)[0] - nm[1,0]*dnm_q[key]- nm[3,0]*dnm_g[key]
    if key=='sx2': dsx2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
    if key == 'sg2': dsg2_ = 0
    if key == 'b': db_ = -(((bg*bq + Cgq)*t)/Exp((b + gq)*t)) - (bg*mq*t)/Exp(b*t) + (bg*mq*t)/Exp((b + gq)*t) + \
	    quad(lambda tau: (Exp(bx - b*t + b*tau + bl*tau + (Cxx + tau*(2*Cxl + Cll*tau))/2.)*(Exp(gq*(-t + tau)) - Exp(-(gq*(t + tau))))*sq2*(-t + tau))/\
	    (2.*gq) + Exp(bx + Cxx/2. - b*t - gq*t + b*tau + bl*tau + Cxl*tau - gq*tau + (Cll*tau**2)/2.)*(-t + tau)*\
	    (bq**2 + Cqq + (Cxq + (-1 + Exp(gq*t))*mq + Clq*tau)*(Cxq + (-1 + Exp(gq*tau))*mq + Clq*tau) + \
          bq*(2*Cxq + (-2 + Exp(gq*t) + Exp(gq*tau))*mq + 2*Clq*tau)),0,t)[0]- nm[1,0]*dnm_q[key]- nm[3,0]*dnm_g[key]
    return {'ml':dml_,'gl':dgl_,'sl2':dsl2_,'mq':dmq_,'gq':dgq_,'sq2':dsq2_\
		,'sx2':dsx2_,'sg2':dsg2_,'b':db_,'sdx2':dsdx2_,'sdg2':dsdg2_}
...
def d_cov_ll(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = (sl2 - Exp(2*gl*t)*sl2 - 4*Cll*gl**2*t + 2*gl*sl2*t)/(2.*Exp(2*gl*t)*gl**2)
    if key=='sl2': dsl2_ = -(-1 + Exp(-2*gl*t))/(2.*gl)
    if key=='mq': dmq_ = 0
    if key=='gq': dgq_ = 0
    if key=='sq2': dsq2_ = 0
    if key=='sx2': dsx2_ = 0
...
def d_cov_lq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = -t*Clq*Exp(-gl*t)*Exp(-gq*t)
    if key=='sl2': dsl2_ = 0
    if key=='mq': dmq_ = 0
    if key=='gq': dgq_ = -t*Clq*Exp(-gl*t)*Exp(-gq*t)
    if key=='sq2': dsq2_ = 0
    if key=='sx2': dsx2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
...
def d_cov_qq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key):
    db_=None; dsdg2_=None; dsdx2_=None; dsg2_=None; dsx2_=None;
    dsq2_=None; dgq_=None; dmq_=None; dsl2_=None; dml_=None; dgl_=None;
    if key=='ml': dml_ = 0
    if key=='gl': dgl_ = 0
    if key=='sl2': dsl2_ = 0
    if key=='mq': dmq_ = 0
    if key=='gq': dgq_ =  (sq2 - Exp(2*gq*t)*sq2 - 4*Cqq*gq**2*t + 2*gq*sq2*t)/(2.*Exp(2*gq*t)*gq**2)
    if key=='sq2': dsq2_ = -(-1 + Exp(-2*gq*t))/(2.*gq) 
    if key=='sx2': dsx2_ = 0
    if key=='sdx2': dsdx2_ = 0
    if key == 'sdg2': dsdg2_ = 0
    if key == 'sg2': dsg2_ = 0
...
def mean_cov_model(m,C,t,ml,gl,sl2,mq,gq,sq2,b):
    ...
    return nm,nC
def dm_dC(m,C,t,ml,gl,sl2,mq,gq,sq2,b,nm,keys):
    """return all derivatives of keys into a dictionary"""
    bx=m[0,0]; bg=m[1,0]; bl=m[2,0]; bq=m[3,0]
    Cxx=C[0,0];Cxg=C[0,1];Cxl=C[0,2];Cxq=C[0,3];Cgg=C[1,1];Cgl=C[1,2];Cgq=C[1,3];Cll=C[2,2];Clq=C[2,3];Cqq=C[3,3]
    dm_dic={};dC_dic={}
    for key in keys:
        dm = np.zeros((4,1));dC=np.zeros((4,4))
        dnm_x = d_mean_x(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)
        dnm_g = d_mean_g(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)
        dnm_l = d_mean_l(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)
        dnm_q = d_mean_q(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)
        dC[0,0] = d_cov_xx(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[0,1] = d_cov_xg(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_x,dnm_g,key)[key]
        dC[0,2] = d_cov_xl(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[0,3] = d_cov_xq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[1,1] = d_cov_gg(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_g,key)[key]
        dC[1,2] = d_cov_gl(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_l,dnm_g,key)[key]
        dC[1,3] = d_cov_gq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,nm,dnm_q,dnm_g,key)[key]
        dC[2,2] = d_cov_ll(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[2,3] = d_cov_lq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[3,3] = d_cov_qq(t,bx,bg,bl,bq,Cxx,Cxg,Cxl,Cxq,Cgg,Cgl,Cgq,Cll,Clq,Cqq,ml,gl,sl2,mq,gq,sq2,b,key)[key]
        dC[1,0] = dC[0,1]; dC[2,0] = dC[0,2];dC[3,0] = dC[0,3]
        dC[2,1] = dC[1,2];dC[3,1] = dC[1,3];dC[3,2] = dC[2,3]
        dm[0,0]=dnm_x[key]
        dm[1,0]=dnm_g[key]
        dm[2,0]=dnm_l[key]
        dm[3,0]=dnm_q[key]
...
def d_div():
    ...
    dsdg2_ = np.zeros((2,2))
    dsdg2_[1,1]=1
    return dsdx2_, dsdg2_
def dD_():
    """Derivative error matrix D"""
    dsx2_ = np.zeros((2,2))
    dsx2_[0,0]=1
    dsg2_ = np.zeros((2,2))
    dsg2_[1,1]=1
    return {'ml':0,'gl':0,'sl2':0,'mq':0,'gq':0,'sq2':0\
		,'sdx2':0,'sdg2':0,'b':0,'sx2':dsx2_,'sg2':dsg2_}
def log_likelihood(y,m,C,sx2,sg2):
    ...
    return log_lik
def d_log_likelihood(y,m,C,dm,dC,sx2,sg2,division,keys):
    """Derivative of the likelihood"""
    d_log_lik={}
    D = np.zeros((2,2));D[0,0]=sx2;D[1,1]=sg2
    dD = dD_()#Error matrix gradient
    if division:#Division gradient
        if 'sdx2' in keys:
            dC['sdx2']=d_div()[0]
        if 'sdg2' in keys:
            dC['sdg2']=d_div()[1]
    ...
    y = y-m[:2,:]
    for key in keys:
        d_log_lik[key] = (1/2*y.T@Si@(dC[key][:2,:2]+dD[key])@Si@y\
                          -1/2*np.trace(Si@(dC[key][:2,:2]+dD[key]))\
                          +dm[key][:2,:].T@Si@y)[0,0]
    return d_log_lik
...
def grad_posterior(y,m,C,sx2,sg2,dm,dC,keys):
    dnm={};dnC={}
    dD = dD_()
    D = np.zeros((2,2));D[0,0]=sx2;D[1,1]=sg2
    Si = np.linalg.inv(C[:2,:2]+D)
    ...
    K = C[:2,:]
    for key in keys:
        dnm[key] = dm[key]+dC[key][:2,:].T@Si@y-K.T@Si@dm[key][:2,:]\
                    -K.T@Si@(dC[key][:2,:2]+dD[key])@Si@y
        dnC[key]  =\
            dC[key]-2*dC[key][:2,:].T@Si@K+K.T@Si@(dC[key][:2,:2]+dD[key])@Si@K
    return dnm, dnC
def initial_der():
    Zer = np.zeros((4,1));Zers = np.zeros((4,4))
    dm = {'ml':Zer,'gl':Zer,'sl2':Zer,'mq':Zer,'gq':Zer,'sq2':Zer\
	  ,'sdx2':Zer,'sdg2':Zer,'b':Zer,'sx2':Zer,'sg2':Zer}
    dC={'ml':Zers,'gl':Zers,'sl2':Zers,'mq':Zers,'gq':Zers,'sq2':Zers\
	,'sdx2':Zers,'sdg2':Zers,'b':Zers,'sx2':Zers,'sg2':Zers}
    return dm,dC
def total_likelihood(Y,m,C,ml,gl,sl2,mq,gq,sq2,b,sx2,sg2,sdx2,sdg2,keys):
    ...
    for i in range(T.shape[0]):
        ...
        grad.append(d_log_likelihood(y,nm,nC,dm,dC,sx2,sg2,div,keys))
        div = False
        # COMPUTE POSTERIOR
        nm,nC= posterior(y,nm,nC,sx2,sg2)
        dm_post,dC_post = grad_posterior(y,m,C,sx2,sg2,dm,dC,keys)
        # NEXT TIME POINT PRIOR NO DIVISION
        if i<T.shape[0]-1:
            dt = T[i+1,0]-T[i,0]
            nm_old = copy.deepcopy(nm);
            nC_old = copy.deepcopy(nC);
            nm,nC = mean_cov_model(nm,nC,dt,ml,gl,sl2,mq,gq,sq2,b)
            dm_ou, dC_ou = dm_dC(nm_old,nC_old,dt,ml,gl,sl2,mq,gq,sq2,b,nm,keys)
            dm = {k: dm_post.get(k, 0) + dm_ou.get(k, 0) for k in set(dm_ou)}
            dC = {k: dC_post.get(k, 0) + dC_ou.get(k, 0) for k in set(dm_ou)}
    return log_lik,pd.DataFrame.from_dict(grad)#.sum(axis=0)
def total_posterior(Y,m,C,ml,gl,sl2,mq,gq,sq2,b,sx2,sg2,sdx2,sdg2,Likelihood,Posterior):
    """TO CCHECK IS WRONG VERSION TO BE UPTDATED"""
